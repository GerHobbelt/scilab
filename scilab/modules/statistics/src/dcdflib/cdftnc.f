...
C
      ...
      IF (.NOT. (df.LE.0.0D0)) GO TO 90
      bound = 0.0D0
      status = -5
      RETURN

   90 CONTINUE
  100 IF (which.EQ.4) GO TO 120
C
C     PNONC
C
      ...
      IF (.NOT. (pnonc.LT.0.0D0)) GO TO 110
      bound = 0.0D0
      status = -6
      RETURN
  ...
  110 CONTINUE
  120 IF (.NOT. (which.EQ.1)) qporq = p .LE. q
      IF ((1).EQ. (which)) THEN
  ...
  130     IF (.NOT. (status.EQ.1)) GO TO 140
  ...
  140     IF (.NOT. (status.EQ.-1)) GO TO 170
          IF (.NOT. (qleft)) GO TO 150
          status = 1
          bound = -inf
          GO TO 160
  ...
  180     IF (.NOT. (status.EQ.1)) GO TO 190
          CALL cumtnc(t,df,pnonc,cum,ccum)
          IF (qporq) THEN
              fx = cum - p
          ELSE
  ...
  190     IF (.NOT. (status.EQ.-1)) GO TO 220
          IF (.NOT. (qleft)) GO TO 200
          status = 1
          bound = zero
          GO TO 210
  ...
  230     IF (.NOT. (status.EQ.1)) GO TO 240
          CALL cumtnc(t,df,pnonc,cum,ccum)
          IF (qporq) THEN
              fx = cum - p
          ELSE
  ...
  240     IF (.NOT. (status.EQ.-1)) GO TO 270
          IF (.NOT. (qleft)) GO TO 250
          status = 1
          bound = 0.0D0
          GO TO 260
